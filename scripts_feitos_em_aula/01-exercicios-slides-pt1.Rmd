...
y_x <- tibble(
  x = runif(60),
    y = 500 + 1 * x + 0.5 * x^2 - 0.3 * x^3+ rnorm(30, sd = 0.01)
) %>% select(y, x)
...
summary(lm(y ~ x+x2+x3, data = y_x))
plot(lm(y ~ poly(x,2), data = y_x))
```
...
y = b0 + b1*log(x)
ggplot(y_x, aes(x = sqrt(x), y = y)) + geom_point() + geom_smooth(method = "lm")
```

```{r}
y_x %>% mutate_all(round, digits = 1)
```
```{r}
meu_modelo <- lm(y ~ x2, data = y_x)
```
